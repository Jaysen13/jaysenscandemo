...
def extract_encrypted_json(param_string: str) -> str | None:
    pattern = r'encryptedJson=([^&]+)'
    # 搜索匹配（忽略大小写，可选）
...
def request_received():
    ...
    print(extract_encrypted_json(jaysendata.body))
    # 提取加密数据
    endata = url_decode(extract_encrypted_json(jaysendata.body))
    print(endata)
    dedata = aes_decrypt(endata)
    # 将解密的数据拼接回去
    jaysendata.body = f"encryptedJson={dedata}"
    # 返回修改后的数据包
...
def handle_request():
    ...
    print(jaysendata.body)
    # 提取加密数据
    dedata = extract_encrypted_json(jaysendata.body)
    print(dedata)
    endata = url_encode(aes_encrypt(dedata))
    # 将加密的数据拼接回去
    jaysendata.body = f"encryptedJson={endata}"
    # 返回修改后的数据包
    return jsonify(jaysendata)
